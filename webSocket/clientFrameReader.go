...
// this is meant for the client to be able to read frames sent from the server because the server frames don't have to be masked

func ClientFrameReader(connection net.Conn) ([]byte, error) {
	for {
		header := make([]byte, 2)
